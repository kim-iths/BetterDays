...
const Button = ({ buttonText, onPress, color, icon, style, innerStyle, iconStyle, textStyle, disabled, filled }) => {
    return (
        <View style={[styles.container, style]}>
            ...
            >
                <View style={[innerStyle, styles.innerContainer, { backgroundColor: filled ? (color ? color : "gray") : "transparent" }]}
                    ...
                    <Text style={[
                        { color: filled ? "white" : (color ? color : null) }, textStyle, styles.text, icon ? styles.textWithIcon : null]
                    }>{buttonText}</Text>
                    {icon ?
